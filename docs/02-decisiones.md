...
| D31 | Empresa, delegaciones y fabricantes | La empresa operadora se organiza en **delegaciones**, cada una con **director, taller, comercial y responsable propios**. **Correos internos** (preparación, cambio, instalación, ruta…) se enrutan por la delegación del expediente, con **herencia del nivel nacional** (p. ej. el **taller puede ser nacional**: se configura una vez y las delegaciones no lo sobrescriben). **Aislamiento por delegación**: cada usuario pertenece a una delegación y solo ve/gestiona los expedientes de la suya; **Dirección/Admin ven todas**. Los **correos de proveedor/fabricante** (solicitud a proveedor y planograma al fabricante) se enrutan por un **registro de fabricantes con correo** según la marca de la máquina. |
| D30 | Precios por canal de pago | Cada línea de tarifa (producto o selección de café) tiene **cuatro precios**: **efectivo, tarjeta empleado, tarjeta bancaria y aplicación**. El **redondeo a 0,05 € se aplica únicamente al precio en efectivo** (tanto en la validación del import como en las revisiones por IPC, D20). |
