...
export const onMailCreado = onDocumentCreated("tenants/{tenantId}/mail/{mailId}", async (event) => {
...
export const onExpedienteEscrito = onDocumentWritten("tenants/{tenantId}/expedientes/{expedienteId}", async (event) => {
    const despues = event.data?.after;
    if (!despues?.exists)
        return;
    const expediente = despues.data();
    const propuesta = expediente.propuestaInversion;
    if (!propuesta?.existe || !propuesta.importe)
        return;
    if (expediente.avisoExcesoEnviado)
        return;
    const coste = (expediente.lineasCoste ?? []).reduce((s, l) => s + (l.importe ?? 0), 0);
    ...
    if (!destinos?.para?.length) {
        logger.warn(`Exceso de inversión en ${expedienteId} pero el tenant ${tenantId} no tiene direcciones para exceso_inversion`);
        return;
    }
    const eur = (n) => new Intl.NumberFormat(config.idioma === "en" ? "en-GB" : `${config.idioma ?? "es"}-ES`, {
        style: "currency",
        currency: "EUR",
    }).format(n);
    ...
    const plantilla = plantillaSnap.exists
        ...
        : {
            ...
            cuerpo: "El coste acumulado de la instalación de {{cliente}} es {{coste}}, y supera la propuesta de inversión ({{inversion}}) en {{exceso}}.\n\nRevisad el expediente.",
        };
    const contexto = {
    ...
    const render = (t) => t.replace(/\{\{(\w+)\}\}/g, (_, c) => contexto[c] ?? `{{${c}}}`);
